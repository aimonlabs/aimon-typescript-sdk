...
import { APIUserAbortError } from 'aimon';
import { Headers } from 'aimon/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Aimon({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['AIMON_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Aimon({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['AIMON_BASE_URL'] = 'https://example.com/from_env';
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Aimon({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Aimon({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Aimon({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Aimon({ apiKey: 'My API Key', fetch: testFetch });
