...
async function load_data() {
  ...
  if (
    ...
    console.log(
      "Embeddings do not exist OR are partially missing. Creating new embeddings..."
    );
...
async function execute(query: string, user_instructions: string[]) {
  ...
  const [
    ...
    instructions,
    generated_respone,
  ] = await am_chat(query, user_instructions, chatEngine);
...
app.post("/api/query", async (req, res) => {
  try {
    ...
    const [generated_respone, aimonResponse] = await execute(
      query,
      user_instructions
    );
    res
      .status(200)
      .json({ message_cb: generated_respone, message_aimon: aimonResponse });
  } catch (error) {
